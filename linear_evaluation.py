...
def inference(loader, model, device):
    feature_vector = []
    labels_vector = []
    for step, (data1, y,mask) in enumerate(loader):
        data1 = data1.to(device)
        #data1=data_selfattention(data1,data1,data1,mask.to(device))

        # get encoding
        with torch.no_grad():
            #x=model.attention_data1.linear_q(x)
            h= model.backbone(data1)
            # x_sh=(x1_sh+x2_sh)/2
            # x_sp=torch.cat((x1_sp,x2_sp),1)
            # h=torch.cat((x_sh,x_sp),1)

            #_, z2, _, h= model(x, x)

        h = h.detach()#从张量图中分离不参与梯度的计算
...
if __name__ == "__main__":
    ...
    n_features = encoder.fc.in_features  # get dimensions of fc layer resnet卷积网络输给全连接网络的维度  全连接网络输出为1000
